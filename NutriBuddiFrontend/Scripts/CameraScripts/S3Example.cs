...
namespace AWSSDK.Examples
{
    public class S3Example : MonoBehaviour
    {
		...
		[System.Serializable]
		public class classificationJSONObject{
			public string classification;

			public static classificationJSONObject CreateFromJSON(string jsonString){
				return JsonUtility.FromJson<classificationJSONObject>(jsonString);
				}
		}
		...
		public GameObject loadingCircle = null;

        void Start()
        {
			loadingCircle.SetActive (false);
			PopUpCanvas.enabled = false;
			SampleFileName = UserSession.session.fileName;
...
//        / </summary>
        public void PostObject()
        {
			...
			//PostBucketButton.enabled = false;
			loadingCircle.SetActive (true);
			PostBucketButton.gameObject.SetActive(false);
            ...
            Client.PostObjectAsync(request, (responseObj) =>
            {
                if (responseObj.Exception == null) //success
                {
                    //ResultText.text += string.Format("\nobject {0} posted to bucket {1}", responseObj.Request.Key, responseObj.Request.Bucket);
						...
						//PopUpCanvas.enabled = true;
						//Debug.Log("PopUpCanvas Status"+PopUpCanvas.enabled);
						string myURL = "http://ec2-54-197-28-169.compute-1.amazonaws.com/classify?img=" + UserSession.session.fileName;
						Debug.Log(myURL);
						BuildWWW myRequest = new BuildWWW();
						StartCoroutine(startMyComplicatedTask(myRequest.BuildRequest(myURL)));
						//SceneManager.LoadScene("PostEatScene");

                }
                else
                {
						loadingCircle.SetActive (false);
						PostBucketButton.gameObject.SetActive(true);
...
//        / </summary>
		...
		IEnumerator startMyComplicatedTask(WWW builtRequest){
			...
			//UserSession.session.currentClassifiedFood = builtRequest.text;
			UserSession.session.currentClassifiedFood = classificationJSONObject.CreateFromJSON(builtRequest.text).classification;
